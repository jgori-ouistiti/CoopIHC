...
class StateElement(numpy.ndarray):
    ...
    :param input_object: value array-like
    ...
    * "raw" --> No data transformation is applied. This is faster than the other options, because the preprocessing of input data is short-circuited. However, this provides no tolerance for misspecified input.
    ...
    def __setitem__(self, key, value):
        ...
        if not isinstance(key, tuple):  # Hack
            value = StateElement._process_input_values(
                value, self.space, self.out_of_bounds_mode
            )
    ...
    def cast(self, other, mode="center"):
        ...
        self_spacetype = self.space.space_type
    ...
    def _discrete2continuous(self, other, mode="center"):
        ...
        if not (
            self.space.space_type == "discrete" and other.space_type == "continuous"
        ):
        ...
        value = shift + ls[self.space._array_bound.squeeze().tolist().index(self[:])]
        return numpy.array(value).reshape((-1, 1))

    def _continuous2discrete(self, other, mode="center"):

        if not (
            self.space.space_type == "continuous" and other.space_type == "discrete"
        ):
            raise AttributeError(
        ...
        if mode == "edges":
            _remainder = (self[:].squeeze() - self.space.low.squeeze()) % (
                _range / other.N
            )
            index = min(
                int(
                    (self[:].squeeze() - self.space.low.squeeze() - _remainder)
                    / _range
                    * other.N
                ),
                other.N - 1,
            ...
            N = other.N - 1
            _remainder = (
                self[:].squeeze() - self.space.low.squeeze() + (_range / 2 / N)
            ) % (_range / (N))

            index = int(
                (
                    self[:].squeeze()
                    - self.space.low.squeeze()
                    - _remainder
                    + _range / 2 / N
                )
                / _range
    ...
    def _continuous2continuous(self, other):
        if not (
            self.space.space_type == "continuous" and other.space_type == "continuous"
        ):
            raise AttributeError(
    ...
    def _discrete2discrete(self, other):
        ...
        if not (self.space.space_type == "discrete" or other.space_type == "discrete"):
            raise AttributeError(
                "Use this only to go from a discrete to a discrete space"
            )
        return other._array_bound.squeeze()[
            self.space._array_bound.squeeze().tolist().index(self[:].squeeze().tolist())
        ]
    ...
    def _tabulate(self):
        ...
        if self.space.space_type == "continuous":
            space[_index] = "Cont{}".format(self.space.shape)
            array = self.view(numpy.ndarray)
        elif self.space.space_type == "discrete":
            space[_index] = "Discr({})".format(self.space.N)
            array = self.view(numpy.ndarray)[0]
        elif self.space.space_type == "multidiscrete":
            space[_index] = "MultiDiscr{}".format(self.space.N)
...
#             )
#         return other._array_bound.squeeze()[
#             self.space._array_bound.squeeze()
#             .tolist()
...
#             if space.space_type == "continuous" and isinstance(
#                 input_object, numpy.ndarray
#             ):
#                 input_object = numpy.atleast_2d(input_object)
#                 if input_object.shape[0] == 1:
#                     input_object = input_object.reshape(-1, 1)

#             for ni, (v, s) in enumerate(
#                 itertools.zip_longest(input_object, space)
#             ):
...
#         if multidiscrete:
#             return [nip.astype(common_dtype) for nip in input_object]
#         else:
#             return input_object[0].astype(common_dtype)
...
def cat_element(n=None, max=None, range=None, init=0, min=0):
    ...
    if range is None:
        input_object = numpy.arange(min, max)
        return StateElement(init, Space(input_object, "discrete"))
