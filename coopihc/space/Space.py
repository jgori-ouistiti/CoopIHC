...
class CatSet(BaseSpace):
    ...
    def __contains__(self, item, mode=None):
        ...
        Checks if item belong to the space. This check is done leniently, according to Numpy __contains__:

        .. code-block:: python

            s = Categorical(array=numpy.array([1, 2, 3], dtype=numpy.int16))
            assert 1 in s
            assert [1] in s
            assert [[1]] in s
            ...
            assert numpy.array([2], dtype=numpy.float32) in s
