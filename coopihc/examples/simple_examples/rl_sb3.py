...
# task_state        position     29                         Numeric() - int64
#                   targets      [ 5 10 14 15 19 21 27 28]  Numeric(8,) - int64
# user_state        goal         10                         Numeric() - int64
# user_action       action       -3                         Numeric() - int64
# assistant_action  action       [1]                        Numeric(1,) - int64
# ----------------  -----------  -------------------------  -------------------
...
class NormalizeActionWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        return {
            "user_action__action": int(
                numpy.around(action * 11 / 2 - TEN_EPSILON64, decimals=0)
            )
        }

    def reverse_action(self, action):
        return numpy.array((action["user_action__action"] - 5.0) / 11.0 * 2).astype(
            numpy.float64
        )
