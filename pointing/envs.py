...
class DiscretePointingTaskPipeWrapper(PipeTaskWrapper):
    ...
    def update_user_state(self, state):
        for key in state:
            ...
            self.bundle.user.state[key]["values"] = state[key]


class DiscretePointingTask(InteractionTask):
    def __init__(
        self, gridsize=(31, 31), number_of_targets=10, mode="gain", layout="uniform"
    ):
        super().__init__()
        self.dim = len(gridsize)
    ...
    def reset(self, dic=None):
        ...
        if self.layout == "uniform":
            selected = numpy.random.choice(
                self.gridindex.reshape(-1),
                size=self.number_of_targets + 1,
                replace=False,
            )
    ...
    def user_step(self, *args, **kwargs):
        state, sumrewards, is_done, reward_list = super().user_step()
        return state, sumrewards, self.is_done_user(), reward_list
    ...
    def assistant_step(self, *args, **kwargs):
        ...
        if self.mode == "position":
            ...
            self.state["position"]["values"] = position + user_action * assistant_action
...
class Screen_v0(InteractionTask):
    def __init__(
        ...
        _screen_size = {
            "name": "screen_size",
            "value": screen_size,
            "meaning": "Size of the screen. The aspect ratio will be computed from the screen size to calculate the height of the screen, the width being [-1,1]",
        }
        _number_of_targets = {
            "name": "number_of_targets",
            "value": number_of_targets,
            "meaning": "number of potential targets from which to choose a goal",
        }
        _target_radius = {
            "name": "target_radius",
            "value": target_radius,
            "meaning": "radius of the circular targets displayed on screen",
        }
        _max_cycle = {
            "name": "max_cycle",
            "value": max_cycle,
            "meaning": "Maximum number of steps possible in this environment before it finished",
        }
        _timestep = {
            "name": "timestep",
            "value": timestep,
            "meaning": "Duration of one step in this environment",
        }
        _mode = {
            "name": "mode",
            "value": mode,
            "meaning": "whether the assistant is expected to work as gain or as positioner. In the first case (gain), the user's action is multiplied by the assistant's action to determine by how much to shift the old position of the cursor. In the second case (position) the assistant's action is directly the new position of the cursor.",
        }
        ...
        self.state["targets"] = StateElement(
            values=[
                numpy.array(
                    [
                        -1 + 2 * i / self.number_of_targets,
                        -1 / self.aspect_ratio
                        + i / self.aspect_ratio / self.number_of_targets,
                    ]
                )
    ...
    def reset(self, dic=None):
        ...
        while (
            len(targets) < self.number_of_targets + 1
        ):  # generate targets as well as a start position
            ...
            if (
                (x > -1 + 2 * radius)
                and (x < 1 - radius)
                and (y > -1 / self.aspect_ratio + 2 * radius)
                and (y < 1 / self.aspect_ratio - 2 * radius)
            ):
                if not targets:
        ...
        radius = [
            numpy.sqrt(numpy.sum((target - numpy.array([-1, -1])) ** 2))
    ...
    def assistant_step(self, assistant_action):
        ...
        if self.mode == "position":
            ...
            user_action = self.bundle.game_state["user_action"]["action"]["values"][0]
            position = self.state["position"]["values"][0]
            newpos = position + user_action * assistant_action

            self.state["position"]["values"] = numpy.clip(
                newpos, self.screen_low, self.screen_high
            )

        # if dist(self.bundle.user.state['goal']['values'][0]) < self.target_radius:
        #     is_done = True
    ...
    def is_done_assistant(self):
        ...
        speed = 0
        threshold = 1
        goal = self.bundle.user.state["goal"]["values"][0]
        position = self.state["position"]["values"][0]
    ...
    def render(self, *args, mode="plot"):
        axgame, axuser, axassistant = args
        if self.ax is not None:
            self.update_position()
        else:
            self.ax = axgame
            self.init_grid(axuser)
            self.ax.set_aspect("equal")
            plt.show(block=False)

    def init_grid(self, axuser):
