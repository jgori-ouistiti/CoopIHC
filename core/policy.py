...
class LinearFeedback(BasePolicy):
    def __init__(self, state_indicator, index, action_state, *args, feedback_gain='identity', **kwargs):
        ...
        _state_indicator = {"name": 'state_indicator', "value": state_indicator,
                            "meaning": 'Indicates which substate of the internal state will be used as action'}
        _index = {"name": 'index', "value": index,
                  "meaning": 'Index that is applied to the state_indicator'}
        _feedback_gain = {"name": 'feedback_gain', "value": feedback_gain,
                          "meaning": 'Gain (K) applied to the selected substate: output of sample = -K @ state[state_indicator][index].'}
        _noise_function = {"name": 'noise_function', "value": self.noise_function.__name__,
                           "meaning": 'A function that is applied to the action to produce noisy actions. The signature of the function is "def noise_function(self, action, observation, *args):" and it should return a noise vector'}
        _noise_args = {"name": 'noise_args', "value": self.noise_args,
                       "meaning": 'args that need to be supplied to the noise function'}

        self.handbook['parameters'].extend(
            [_state_indicator, _index, _feedback_gain, _noise_function, _noise_args])
    ...
    def sample(self):
        ...
        if isinstance(self.feedback_gain, str):
            if self.feedback_gain == 'identity':
                self.feedback_gain = - \
                    numpy.eye(max(substate['values'][0].shape))
...
class BIGDiscretePolicy(BasePolicy):
    def __init__(self, action_state, operator_policy_model, assistant_action_space=[None]):
        try:
            assistant_action_possible_values = [
                list(range(assistant_action_space[0].n))]
        except AttributeError:
            assistant_action_possible_values = None

        super().__init__(action_state, action_space=assistant_action_space,
                         action_set=assistant_action_possible_values)
        ...
        _action_state = {"name": 'action_state', "value": action_state,
                         "meaning": 'Reference to the action_state, usually from the game_state'}
        _assistant_action_space = {"name": 'assistant_action_space', "value": assistant_action_space,
                                   "meaning": 'The space in which the assistant can take actions'}
        _operator_policy_model = {"name": 'operator_policy_model', "value": operator_policy_model,
                                  "meaning": 'An instance of a Policy class (or Subclass)'}

        self.handbook['render_mode'].extend(['plot', 'text'])
        self.handbook['parameters'].extend(
            [_action_state, _assistant_action_space, _operator_policy_model])
    ...
    def PYy_Xx(self, operator_action, assistant_action, potential_states, beliefs):
        ...
        for potential_state, belief in zip(potential_states, beliefs):
            pYy__Xx += self.operator_policy_likelihood_function(
                operator_action, potential_state)*belief
        return pYy__Xx
    ...
    def HY__Xx(self, potential_states, assistant_action, beliefs):
        ...
        for operator_action in self.operator_action_set:
            pYy_Xx = self.PYy_Xx(
                operator_action, assistant_action, potential_states, beliefs)
            if pYy_Xx != 0:
    ...
    def HY__OoXx(self, potential_states, assistant_action, beliefs):
        ...
        for operator_action in self.operator_action_set:
            for potential_state, belief in zip(potential_states, beliefs):
                ...
                pYy__OoXx = self.operator_policy_likelihood_function(
                    operator_action, potential_state)
    ...
    def find_best_action(self):
        ...
        IG_storage = [self.IG(action, observation, beliefs)
...
class RLPolicy(BasePolicy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        action_state = State()
        action_state['action'] = copy.deepcopy(getattr(getattr(getattr(
            self.training_env.unwrapped.bundle, 'operator'), 'policy'), 'action_state')['action'])
